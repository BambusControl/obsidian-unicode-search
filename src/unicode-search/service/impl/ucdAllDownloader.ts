...
/**
 * Unicode Character Database Downloader for all characters
 */
export class UCDAllDownloader implements CharacterDownloader {
		...
		return new Promise((resolve, reject) => {
			const completeFn = (results: ParseResult<ParsedData>): void => {
				...
				const unicodeCharacters = results.data
					...
					.filter(char => UCDAllDownloader.charFilter(char))
					.map(pch => UCDAllDownloader.intoUnicode(pch));
